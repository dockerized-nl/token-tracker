...
struct ClaudeDashboardView: View {
            ...
            VStack(alignment: .leading, spacing: 20) {
                ...
                LazyVGrid(columns: cols, spacing: 14) {
                    ...
                    StatTile(title: "Sessions",
                             ...
                             subtitle: "\(store.records.count) messages",
                ...
                if store.records.isEmpty {
                    HintBox(icon: "doc.text.magnifyingglass",
                            title: store.isScanning ? "Scanning logs…" : "No usage found yet",
                            message: "Reading Claude Code logs from \(store.logsRoot.path). Use Claude Code, then press Refresh.")
                } else {
                    perDayChart
                    perHourChart
    ...
    private var statusLine: String {
        var parts: [String] = ["\(store.fileCount) log files"]
    ...
    private var perDayChart: some View {
        ...
        return Card {
            VStack(alignment: .leading, spacing: 12) {
                ...
                .chartXAxis { AxisMarks(values: .stride(by: .day, count: 5)) { v in
                    AxisGridLine().foregroundStyle(Theme.cardStroke)
